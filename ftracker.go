...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInMsk := meanSpeed(action, duration) * kmhInMsec
	spentCalories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInMsk, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return spentCalories
}
